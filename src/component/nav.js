...
export default function Nav(props) {
            ...
            rowSet.push( productState.slice(i, i+4).map((category, n) => {
                ...
                return(
                      <div key={n} className="col">
                        ...
                        <Link to="#" className="dropdown-item dropMenuHead">{category.product}</Link>
                        <div className="dropdown-divider"></div>
    
                        {category.products.map( (product,pid) => {
                            return(
                                <Link key={pid} className="dropdown-item" to={"/product/id="+product.id}>
                                    {product.name}
                                </Link>
    ...
    // update Set Searchstate
    
    const handleOnChangeEvent = (e) => {
        dispatch({type:"SEARCHKEY", payload: DOMPurify.sanitize(e.target.value) })
        console.log("Sanitized: ",DOMPurify.sanitize(e.target.value));
    }
